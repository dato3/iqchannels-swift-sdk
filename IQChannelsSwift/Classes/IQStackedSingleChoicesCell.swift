...
protocol IQStackedSingleChoicesCellDelegate: AnyObject {
    func stackedSingleChoicesCell(_ cell: IQStackedSingleChoicesCell, didSelectOption singleChoice: IQSingleChoice)
}

final class IQStackedSingleChoicesCell: UICollectionViewCell {
    ...
    weak var stackedSingleChoicesDelegate: IQStackedSingleChoicesCellDelegate?
...
//    }
    ...
    func setSingleChoices(_ singleChoices: [IQSingleChoice]) {
        ...
        for singleChoice in singleChoices {
            let button = getNewButton(withTitle: singleChoice.title ?? "")
    ...
    private func getNewButton(withTitle title: String) -> UIButton {
        ...
        button.backgroundColor = UIColor(red: 173.0/255, green: 184.0/255, blue: 191.0/255, alpha: 1)
    ...
    @objc private func buttonAction(_ button: UIButton) {
        if let index = buttonsArray.firstIndex(of: button) {
            stackedSingleChoicesDelegate?.stackedSingleChoicesCell(self, didSelectOption: singleChoices[index])
        }
    }
    ...
    private func layoutStackView(with attributes: CustomMessagesCollectionViewLayoutAttributes) {
        let y = messageContainerView.frame.maxY + attributes.messageContainerPadding.bottom + 4
        let avatarPadding = attributes.avatarLeadingTrailingPadding
        ...
        stackView.frame = CGRect(origin: origin, size: attributes.additionalContainerSize)
    }
